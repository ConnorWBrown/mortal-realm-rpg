...
In hand-authored `grid`s, the convention for what's just outside a door is: stub doors (no `to`, i.e. unmodeled exterior exits) get `grass` tiles on their outside face; doors that `to` another room get `void` tiles instead (see `eugene-kitchen.json`/`eugene-livingroom.json` for examples, and the "Reshape exit grass into an L perpendicular to the door" commit for the exact grass-tile pattern used around a stub door).
...
Rendering is layered per frame in `render()`: clear → room tiles → boxes → player → effect HUD → room label → door labels (proximity badges naming a door's destination, `renderDoorLabels` in `src/render/hud.ts`) → sidebar (quests/inventory/effects tabs, `src/ui/sidebar.ts`) → modal menu stack (`src/ui/menu.ts`, used for box contents/potion application). Sprites come from Kenney tilesets under `public/assets/`, loaded async by `src/render/assets.ts`; rendering must tolerate `assets` being `null` before load completes.

All in-world text (room labels, door labels, etc.) goes through the hand-rolled pixel font in `src/ui/menu.ts` (`drawText`/`textWidth`), which draws each glyph as 1x1 `fillRect`s — pass it **integer** `x`/`y` or the canvas anti-aliases those rects and the text renders blurry. Watch for this when centering text (e.g. `x + (TILE_SIZE - textWidth(label)) / 2` needs `Math.round`).
...
When testing movement/interaction via browser automation (e.g. the `claude-in-chrome` MCP tools), synthetic key presses sent through the `computer` tool's `key` action don't reliably reach the game's `window` keydown listener (`src/game/input.ts`), so the player often just doesn't move. Two more reliable ways to get the player into a specific spot for a screenshot: (1) `javascript_tool` to `window.dispatchEvent(new KeyboardEvent('keydown', {key: 'ArrowRight', bubbles: true}))` (with a matching `keyup`), or (2) simpler for a one-off position — write `localStorage['mortal-realm:player:v1']` directly (`{x, y, facing, activeEffects: [], inventory: [], quests: [], now: ''}`, world coords = room's `worldOrigin` + local door/tile coords) and reload. Remember to clear that key afterward so you don't leave test state behind.
